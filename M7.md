...
Note when we have a [pointer to a struct](https://en.wikipedia.org/wiki/Struct_(C_programming_language)#Pointers_to_struct), we can use `->` instead of `.` to access its members.
