...
Do [XOR Swap](https://en.wikipedia.org/wiki/XOR_swap_algorithm) on the struct's pointers. Notive in `void swap(Point *&p, Point *&q)`, the pointers are passed by reference. The example also demonstrated [dynamic memory management](https://www.geeksforgeeks.org/new-vs-malloc-and-free-vs-delete-in-c/) in both C and C++. 
