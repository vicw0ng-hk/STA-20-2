...
int main() {
  ...
  while (m * m + 1 <= 2 * N) {
    // now loop on n from 1 to m-1 (m > n > 0)
      for (int n = (int)ceil((float)m/(1.0+sqrt(2.0))); n < m; ++n) { // make sure s1 < s2
        if (gcd(m, n) != 1)
          continue;
...
The deciding factor here is the triple-nested for loop. Together they go through `O(nlogn)` Pythagorean triples without miss. Hence time complexity is `O(nlogn)`.

(If we only need to find primitive Pythagorean triples, it would be `O(n)`)
...
Time complexity: <code>max{O(nlogn), O(nlogn * log(nlogn))} = O(nlogn * (logn + log(logn))) = O(nlog<sup>2</sup>n)</code>
