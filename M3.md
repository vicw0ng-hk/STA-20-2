...
```
Time Complexity: <code>O(n<sup>1.5</sup>)</code> (2 nested loops, each go through `O(sqrt(n))` values, 3rd loop goes through `N / h = O(sqrt(n))` values)

(If we only need to find primitive pythagorean triples, it's `O(n)`.)
...
= max{O(n<sup>1.5</sup>), O(nlogn * logn + nlogn * log(logn))} 
